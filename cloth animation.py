...
P = [[x*20+W/4, y*20+100, x*20+100, y*20+100, y==0] 
     for y in range(30) for x in range(50)]

S = [[i, i+1, 1] for i in range(len(P)) if (i+1)%50] + \
    [[i, i+50, 1] for i in range(len(P)-50)]

while True:

    if any(e.type == pygame.KEYDOWN and e.key == pygame.K_ESCAPE
    ...
    for _ in range(6):
        for i in [k for k in S if k[2]]:
            p1, p2 = P[i[0]], P[i[1]]

            dx, dy = p2[0]-p1[0], p2[1]-p1[1]
            d = math.hypot(dx, dy) or 0.1

            if d > 100 or (md[2] and math.hypot((p1[0]+p2[0])/2-mx,
                                                (p1[1]+p2[1])/2-my) < 15):
                i[2] = 0
                continue
